...
def moveindex(tensor1, axis, position=-1):
    """ Moves the index in the tensor to the given position. If position = -1
...
def effective_vector(site, constraint, MPS, left, right, direction):
    ...
    if site == 0:
        ...
        B = constraint.get_matrix(0)
        B = np.reshape(B, (dim, np.size(B, axis=2)))
        Ceff = np.tensordot(np.conj(B), right[1], axes=(1, 1))
        Ceff = np.reshape(Ceff, (1, dim*np.size(right[1], axis=0)))
    elif site == length - 1:
        # Contract the left block with the remaining constraint site at L - 1
        B = constraint.get_matrix(site)
        B = np.reshape(B, (np.size(B, axis=0), dim))
        Ceff = np.tensordot(left[site - 1], np.conj(B), axes=(1, 0))
        Ceff = np.reshape(Ceff, (1, np.size(left[site - 1], axis=0)*dim))
    else:
        # Contract the left and right block with the remaining site
        B = constraint.get_matrix(site)
        Ceff = np.tensordot(left[site-1], np.conj(B), axes=(1, 0))
        Ceff = np.tensordot(Ceff, right[site+1], axes=(2, 1))
        Ceff = np.reshape(Ceff, (1, np.size(left[site - 1], axis=0)*dim*
...
def sector_operator(length, black=0):
    I = np.identity(2)
    n = np.array([[0, 0], [0, 1]])
...
def variational_sweep(H, initial, sector, constraint = None,
                      constraint_multiplier = 100, epsilon=10**-10,
                      min_sweeps=2, max_sweeps = 100, k=1, sector_mult=1, black=0):
    ...
    # Define the sector and sector squared operators
    S = sector_operator(length, black)
    S2 = product(S, S)   
    ...
    # Sector constraints
    S_left, S_right = initiate_effective_operator(S, psi)
    S2_left, S2_right = initiate_effective_operator(S2, psi)
    ...
    while cont or sweeps < min_sweeps:
        ...
        for site in sites:    
            ...
            if constraint is not None:
                ...
                # Use this to form the constraint 
                const_matrix = np.matmul(np.transpose(Ceff), Ceff)
                
                # Add this to the hamiltonian
                Heff = Heff + constraint_multiplier * const_matrix
            
            # Calculate effective matrix for sectors and adjust hamiltonian
            S_left, S_right, Seff = effective_operator(site, S, psi, S_left,
                                                       S_right, direction)
            S2_left, S2_right, S2eff = effective_operator(site, S2, psi, S2_left
                                                          , S2_right, direction)
            Heff += sector_mult*(S2eff - 2*sector*Seff)
        ...
        #Sexp = expectation(S, psi)
        #S2exp = expectation(S2, psi)
        #print(Sexp)
        #print(S2exp)
    ...
    return [psi, Hexp, Hvar, Sexp, Svar]
